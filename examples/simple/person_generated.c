...
int person_copy(Person* dest, const Person* src) {
    ...
    if (src->children != NULL && src->num_children > 0) {
        dest->children = malloc(src->num_children * sizeof(*src->children));
...
void person_print(const Person* self, FILE* out, int depth) {
    ...
    fprintf(out, "%zu", self->num_children);
...
bool person_deep_equal(const Person* a, const Person* b) {
    ...
    if (a->children != b->children) {
        ...
        for (size_t i = 0; i < a->num_children; i++) {
            if (!person_deep_equal(a->children[i], b->children[i])) return false;
        }
    }
...
uint64_t person_hash(const Person* self) {
    ...
    if (self->children != NULL) {
        ...
        for (size_t i = 0; i < self->num_children * sizeof(*self->children); i++) {
            hash ^= bytes[i];
            hash *= fnv_prime;
        }
    }
